"""Architecture-independent optimization and evaluation loop."""
...
from .hard_negatives import HardNegativeRuntime
from .logger import PrettyLogger
from .objectives import ModelProvidedLoss, TrainingObjective
from .runtime import make_scheduler, resolve_precision
...
class Trainer:
    ...
    def _resume(self, path: Path) -> None:
        ...
        self.logger.resumed(path, self.step)
    ...
    def _enable_compile(self) -> None:
        ...
        if compile_encoder is None:
            self.logger.compile_status(
                False,
                warning="model has no compile_encoder hook",
            )
            return
        try:
    ...
    def fit(self) -> Path:
        ...
        with self.logger.training_progress(
            ...
            task = progress.add_task(
                ...
                completed=min(self.step, self.total_steps),
            )
            for epoch in range(self.start_epoch, self.config.epochs):
                ...
                for batch_index, chunk in enumerate(self.train_loader):
                    ...
                    if (
                        ...
                        self.save_checkpoint(f"step_{self.step:08d}.pt", epoch)
        ...
        self.logger.training_finished(self.step, path)
        return path
